// Matrix.cpp (fixed error handling)
#include "Matrix.h"
#include "Utilities.h"
#include <iostream>
#include <cmath>
...
Matrix::Matrix() : matrix(new int[0]), columns(0), rows(0) {}
...
bool Matrix::operator==(const Matrix &matrix) {
    ...
    for(int i = 0; i < rows * columns; i++) {
        if(matrix.matrix[i] != this->matrix[i]) return false;
    }
    return true;
}
...
double Matrix::CalcFrobeniusNorm(const Matrix& matrix) {
    ...
    for(int i = 0; i < matrix.rows; i++) {
        for(int j = 0; j < matrix.columns; j++) {
            int value = matrix.matrix[i * matrix.columns + j];
            sum += value * value;
        }
    }
    return sqrt(sum);
